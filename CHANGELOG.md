...
- **Upgrade-валидация**: проверка «все ноды Ready» больше не падает на закордоненных нодах (`Ready,SchedulingDisabled` ломал текстовый разбор `kubectl get nodes`). Готовность определяется по `status.conditions[Ready]` из JSON; cordoned-ноды допустимы, fail перечисляет только реально неготовые.
...
- **etcd-бэкап в upgrade-флоу**: имя файла дополнено таймстемпом (`snapshot-pre-<версия>-<YYYYMMDDHHMMSS>.db`); хранятся последние три копии, более старые (включая legacy `snapshot-pre-<версия>.db`) удаляются автоматически после успешного снапшота.
